    ...
    return GameState(
        ...
        teams=[
            Team(
                ...
                tyre_supplier_yearly_cost=450_000,
            )
        ],
        drivers=[
...
def test_calculate_race_cost_splits_tyre_supplier_cost():
    manager = TyreSupplierCostManager()
    assert manager.calculate_race_cost(yearly_cost=450_000, races_in_season=16) == 28_125


def test_charge_for_race_adds_tyre_supplier_transaction():
    state = create_state()
    manager = TyreSupplierCostManager()
    ...
    assert charge.applied_cost == 225_000
    txs = [t for t in state.finance.transactions if t.category == TransactionCategory.TYRE_SUPPLIER]
    assert len(txs) == 1
    assert txs[0].amount == -225_000
