...
def load_drivers(cursor, start_year: int) -> tuple[List[Driver], Dict[str, Driver]]:
    ...
    cursor.execute(
        f"SELECT id, name, age, country, wage, pay_driver, {speed_expr} AS speed, {consistency_expr} AS consistency, {qualifying_expr} AS qualifying, {race_starts_expr} AS race_starts, {wins_expr} AS wins, {podiums_expr} AS podiums, {poles_expr} AS poles, {fastest_laps_expr} AS fastest_laps, {championships_expr} AS championships, {contract_length_expr} AS contract_length "
    ...
    for row in cursor.fetchall():
        ...
        race_starts = row[9] if row[9] is not None else 0
        wins = row[10] if row[10] is not None else 0
        podiums = row[11] if row[11] is not None else 0
        poles = row[12] if row[12] is not None else 0
        fastest_laps = row[13] if row[13] is not None else 0
        championships = row[14] if row[14] is not None else 0
        contract_length = row[15] if row[15] is not None else 2
        driver = Driver(
            id=row[0],
            name=row[1],
            age=row[2],
